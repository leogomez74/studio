...
| Fecha | Mejora |
|-------|--------|
| May 2026 | **Migración legacy CrediPEP — comando `migrar:creditos-legacy`**: comando Artisan que migra créditos+plan+pagos+asientos históricos desde la BD legacy `progrex_new` (conexión `legacy` aditiva en `config/database.php`). REUSA `ImportacionCreditoCreator::crear()` sin tocar el motor. Deductora híbrido refinado (comprobante coope CSG/CS/CNA→planilla, numérico suelto→deductora del crédito, M-SJ/AJ/FND→ventanilla). Fecha asiento híbrida (período del comprobante o MOV_FECHA). numero_operacion=ID_SOLICITUD, categoria=CODIGO (preserva producto). Guard de anomalías (salta cuota<=0/monto<=0/cuota>monto). Alcance: 2.926 A+C, 408 anómalos, 2.518 migrables. Validado en dry-run (MCEL/4693, PERF/4569, PE36/73). **Pendiente ejecutar** (requiere ventana ERP rate-limit) |
...
| Mar 2026 | **Backend — Robustez y Privacidad**: Fix `Undefined array key "notes"` con null coalescing; privacidad reforzada en `recent` (mensajes directos solo visibles para emisor/receptor). |
...
| 2026-03-17 | **Multi-Assignee en Automatizaciones de Tareas**: Tabla pivote `task_automation_assignees` para múltiples responsables por automatización. `Task::createFromAutomation()` crea una tarea por responsable. 7 controllers actualizados (Lead, Opportunity, Credit, Analisis, Propuesta, PaymentVerification, CreateMissingCreditTasks). Frontend: multi-select con Popover+Checkbox en TareasAutomationTab (badges removibles). Backward compat: `getAssigneeIds()` fallback a legacy `assigned_to` |
| 2026-03-17 | **Automatizaciones de autorización admin**: 3 nuevos eventos configurables en Tareas Automáticas: `payment_reversal_request` (anulación de abono, antes hardcoded a permiso cobros.archive), `saldo_reintegro_request` (reintegro de saldo, antes hardcoded a permiso cobros.assign), `reward_redemption_request` (canje de recompensa crea tarea para admin). Con fallback a búsqueda por permisos si no hay automation configurada. Frontend: 10 cards en TareasAutomationTab |
| 2026-03-17 | **4 nuevas automatizaciones de flujos de crédito**: `cancelacion_anticipada` — migrada de hardcoded a automation configurable (CancelacionService, fallback: assigned_to del crédito). `credit_mora` — tarea de seguimiento de cobro al entrar en mora por primera vez (MoraService). `abono_extraordinario` — tarea para verificar plan y notificar cliente tras abono extraordinario (AbonoService). `credit_cerrado` — tarea de archivo documental al cerrar crédito por cancelación anticipada o abono que deja saldo en 0 (CancelacionService + AbonoService). PaymentVerificationController mejorado: ahora copia workflow_id y checklist de la automation. Frontend: 14 cards en TareasAutomationTab |
| 2026-03-17 | **5 automatizaciones de inversiones**: `investment_created` — formalizar acuerdo al crear inversión (InvestmentController::store). `investment_renewed` — verificar términos al renovar (InvestmentController::renew). `investment_liquidated` — procesar liquidación anticipada (InvestmentController::liquidate). `investment_cancelacion_total` — completar cancelación total con/sin intereses (InvestmentController::cancelacionTotal). `investment_finalized` — archivar expediente al finalizar inversión, centralizado en `triggerFinalizedAutomation()` privado en InvestmentService + InvestmentCouponController (4 puntos de auto-finalización). Frontend: 19 cards en TareasAutomationTab |
| 2026-03-18 | **Gamificación — Admin CRUD Settings funcional**: Página Settings de Rewards conectada a API real (antes 100% mock). 12 endpoints CRUD en `GamificationConfigController` para catálogo/badges/challenges. Frontend: 5 tabs funcionales con diálogos crear/editar, toggle activar/desactivar, protección eliminación con relaciones |
| 2026-03-18 | **Gamificación — Optimización performance + Layout mejorado**: (1) Backend: N+1 eliminados en ChallengeService (withCount+eager load participations), RewardService (loadCount badges), LeaderboardService (avatar en select). (2) Endpoint `/rewards/dashboard` consolidado: retorna summary+badges+leaderboard+challenges+actividad en 1 request (antes 4 requests secuenciales). Cacheado 60s via CacheService. (3) Frontend: `GamificationDashboard` ahora usa 1 solo hook (`useRewardsDashboard`). Nuevo layout: Hero card (gradiente amber con nivel+puntos+racha), 4 stats cards, layout 3 columnas (contenido principal 2/3 + sidebar 1/3), CTA catálogo en sidebar. Componente reducido y más limpio |
| 2026-03-18 | **Fix Catálogo de Recompensas**: CatalogService y CatalogController público usaban columnas antiguas (`cost`, `currency`, `level_required`) que fueron renombradas/eliminadas en migración. Corregido a `points_cost`, eliminado `currency`, `level_required` → `requirements['min_level']`. Model: `getIsAvailableAttribute` y `getInStockAttribute` no manejaban stock=-1 (ilimitado). `scopeAvailable` igual. Frontend: tipos actualizados a snake_case del API, función `redeemItem()` activada (estaba comentada con setTimeout simulado). CatalogCategory expandido: +discount +general |
| 2026-03-18 | **Gamificación — Acciones de Negocio estilo Duolingo**: Evento genérico `BusinessActionPerformed` + listener `AwardBusinessPoints` (queued) conecta todos los flujos CRM con el sistema de recompensas. 10 acciones configuradas en `config/gamification.php` sección `actions`: lead_created(25pts/10xp), lead_converted(100pts/30xp), opportunity_created(30pts/12xp), opportunity_status_advanced(25pts/10xp), opportunity_won(200pts/50xp), analisis_created(40pts/15xp), credit_created(150pts/40xp), payment_recorded(15pts/8xp), planilla_uploaded(100pts/25xp), investment_created(75pts/20xp). Dispatch inyectado en 6 controllers: LeadController(store+convertToClient), OpportunityController(store+updateStatus), AnalisisController(store), CreditController(store), CreditPaymentController(store+upload), InvestmentController(store). Cada acción también alimenta rachas (StreakService) y verifica badges (BadgeService) |
| 2026-03-18 | **Gamificación — Dashboard con gráficos**: Dashboard ya no es estático — 3 gráficos con recharts: AreaChart actividad semanal (últimos 7 días de puntos), BarChart horizontal top 5 acciones CRM con más puntos, PieChart donut distribución de badges por rareza. Todos muestran estructura visual incluso sin datos (ejes, labels, placeholder ring). Usa `useRewardsAnalytics('week')` + `ChartContainer` del proyecto. Layout reorganizado: gráficos en columna principal, donut badges en sidebar |
| 2026-03-18 | **Auditoría Rewards — 10 fixes críticos**: (1) RewardTransaction `$fillable` usaba `source_type/source_id` pero DB tiene `reference_type/reference_id` — corregido + `currency` agregado. (2) PointsEarned event: constructor con 5 params pero llamado con 2 — refactorizado a `($rewardUser, $transaction, $context)` con getters. (3) HandlePointsEarned listener actualizado a usar getters del evento. (4) RedemptionService: relación `approvedBy` → `processedByUser`. (5) RewardChallengeParticipation: `rewards_claimed` faltaba en fillable/casts. (6) Leaderboard: `currentUserId=1` hardcoded → `useAuth()`. (7) Badges: progress 0-1 mostrado sin ×100 → normalización automática. (8) Hooks `useRewardsBalance/useTransactionHistory/useBadgeProgress/useRedemptions`: errores silenciados con console.error → error state expuesto. (9) CatalogService/CatalogController: columnas antiguas (cost/currency/level_required). (10) Model RewardCatalogItem: stock=-1 no manejado como ilimitado |
| 2026-03-17 | **2 automatizaciones de operaciones**: `planilla_anulada` — verificar saldos post-anulación de planilla (PlanillaUploadController::anular, incluye deductora, pagos reversados, monto y motivo en detalles). `lead_inactivity_alert` — seguimiento de leads/oportunidades inactivos (CheckLeadInactivity command cron diario, incluye conteo y nombres de leads inactivos). Frontend: 21 cards en TareasAutomationTab |
...
| 2026-04-06 | **Script inserción masiva DB**: `scripts/insertar_carga_masiva.py` — lee CSV (semicolon, UTF-8) exportado de la plantilla xlsx, inserta en orden: persons → opportunities → analisis → credits → plan_de_pagos (sistema francés idéntico al observer Laravel). Lookups automáticos de `tasa_id` y `deductora_id`. IDs generados replicando lógica Laravel (`YY-XXXXX-101-OP`, `YY-ID-01-CRED`). Reporte final con éxitos/errores por fila |
| 2026-04-07 | **SQL carga masiva phpMyAdmin — 17 registros producción**: `scripts/insertar_carga_masiva.sql` — SQL listo para pegar en phpMyAdmin con 17 registros reales (enero–marzo 2026). Flujo: persons → opportunities → analisis → credits (sin plan_de_pagos). `tasa_id` = FK a cualquier tasa activa; `tasa_anual` = valor exacto del CSV. Correcciones aplicadas: decimales con coma → punto, encoding UTF-8 (á/é/í/ó/ú/ñ), tel_casa "0"→NULL, fechas YYYY-MM→último día mes, fecha "20/022026"→2026-02-20, reg 1 monto=450000 tasa=54.36. Deductoras: regs 9/11/17 = COOPESERVICIOS; resto = COOPENACIONAL |
| 2026-04-06 | **Módulo Roles y Permisos — rediseño completo**: 20 → 24 módulos, UI chips por módulo (no tabla fija), sub-módulos de config granulares, "Ver propias/Ver todas" para tareas, migración para Colaborador y Finanzas, corrección de bypass `tareas` en sidebar |
| 2026-04-06 | **Archivado de Clientes**: `PATCH /api/clients/{id}/toggle-active` + `Person::findOrFail()` (bypass global scope Lead), botón Archivar por fila en CRM clientes, SweetAlert2, fix `handleRestore()` detecta lead vs client por `lead_status_id` |
| 2026-04-08 | **ERP HMAC — Unificación makeRequest()**: `ErpAccountingService` ahora usa `makeRequest(method, endpoint, payload)` para todas las peticiones HTTP (antes duplicado en sendWithRetry y createAccount). GET requests usan `$body=''` (sha256('') que espera el ERP). Fix `json_encode` false lanza Exception explícita. `testConnection()` retorna éxito si ERP_SERVICE_TOKEN configurado sin hacer request real (GET /health daba 401 en entorno de prueba) |
| 2026-04-08 | **Fix TypeScript build: VendedorRow.posicion null**: `posicion: number` → `number \| null` en VendedoresTable.tsx. `MedalBadge` maneja null con `<span>—</span>`. Fix necesario para deploy en dev |
| 2026-04-08 | **Endpoint /api/health/env/detail**: Retorna detalles de configuración de variables de entorno. Solo admin autenticado (`auth:sanctum + admin + throttle:10,1`). Separado de `/health/env` que es público |
| 2026-04-08 | **Auditoría Seguridad — Mass Assignment + Hidden Fields**: `Person.$guarded = ['*']` (base model bloquea todo, subclases definen su propio `$fillable`). `Investor.$hidden = ['erp_account_key', 'erp_account_key_prestamos', 'erp_account_key_intereses']` |
| 2026-04-08 | **SecurityHeaders Middleware global**: `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy`, `HSTS` (solo HTTPS). Registrado globalmente en `bootstrap/app.php` con `$middleware->append()` |
| 2026-04-08 | **Rate limiting adicional**: activity-logs con `admin + throttle:10,1`. Export endpoints (investment, credit-payments, planilla, accounting-entries) con `throttle:10-20,1` |
