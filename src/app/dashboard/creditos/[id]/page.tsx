                      ...
                      {credit.plan_de_pagos && credit.plan_de_pagos.length > 0 ? (() => {
                        ...
                        return rows.map((row) => {
                          ...
                          <TableRow key={payment.id} className="hover:bg-muted/50">
                            ...
                            <TableCell className="text-xs text-right font-mono text-blue-700">{formatCurrency(['Pagado','Pagada','Parcial'].includes(payment.estado||'') ? Number(payment.saldo_nuevo||0) : Number(payment.saldo_anterior||0) + Number(payment.int_corriente_vencido||0) + Number(payment.interes_moratorio||0))}</TableCell>
