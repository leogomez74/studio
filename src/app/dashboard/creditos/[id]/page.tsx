...
function CreditDetailClient({ id }: { id: string }) {
  ...
  const generatePDFContent = (doc: jsPDF, creditData: CreditItem, date: string) => {
    ...
    const cuotasPagadas = (creditData.plan_de_pagos || []).filter((p) => ['Pagado', 'Pagada'].includes(p.estado || ''));
    ...
    if (cuotasPagadas.length > 0) {
      ...
      const paymentRows = cuotasPagadas.map((p) => [
        p.numero_cuota,
        formatDate(p.fecha_corte),
                      ...
                      {credit.plan_de_pagos && credit.plan_de_pagos.length > 0 ? (() => {
                        ...
                        return rows.map((row) => {
                          ...
                          <TableRow key={payment.id} className={`hover:bg-muted/50 ${isVigente ? 'bg-gray-50/60 italic text-muted-foreground' : ''}`}>
                            ...
                            <TableCell className="text-xs text-right font-mono text-blue-700">{formatCurrency(['Pagado','Pagada','Parcial'].includes(payment.estado||'') ? Number(payment.saldo_nuevo||0) : payment.estado==='Mora' ? Number(payment.saldo_anterior||0) + Number(payment.interes_corriente||0) + Number(payment.int_corriente_vencido||0) + Number(payment.interes_moratorio||0) : Number(payment.saldo_anterior||0))}</TableCell>
